...
section("4 a share link opens one casting call, and only that one");
{
  ...
  check("and says who holds the data, for how long, and what you can do", (await p.getByRole("heading", { name: "Your data" }).count()) === 1 && (await p.getByText(/data controller/).count()) > 0 && (await p.getByText(/30 days after the production finishes/).count()) > 0 && (await p.getByText(/Information Commissioner/).count()) > 0);
